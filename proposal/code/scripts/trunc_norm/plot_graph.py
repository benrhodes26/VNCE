...
for i, file in enumerate(os.listdir(load_dir)):
    ...
    nce_missing_mse = loaded['nce_missing_mse']
    frac_to_mse_dict[str(frac)] = [vnce_mse, nce_missing_mse]
    if file == 'frac0':
...
ax.plot(np.array(fracs), np.array(nce_missing_mses), label='NCE')
ax.plot((0, 1), (0.99782, 0.99782), label='MSE of initial params')
