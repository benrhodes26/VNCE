...
def make_quantile_to_run_dict(runs, sample_sizes):
    ...
    for the given estimation method. The list contains one run (i.e triple) for every sample size.
    """

    quantile_to_run_dict = {}
    for j, sample_size in enumerate(sample_sizes):
        runs_for_sample = runs[str(sample_size)]

        # sort the runs by mean-squared error of final parameter estimate
        sorted_runs_for_sample = sort_by_mse(np.array(runs_for_sample))

        # save the 0.1, 0.5 & 0.9 quantiles
        save_deciles(quantile_to_run_dict, sorted_runs_for_sample)
...
def unroll_runs(runs):
    ...
    for key, val in runs.items():
        for i, run in enumerate(val):
            unrolled_dict['sampleSize:{}_run:{}_thetas'.format(key, i)] = run[0]
            unrolled_dict['sampleSize:{}_run:{}_times'.format(key, i)] = run[1]
            unrolled_dict['sampleSize:{}_run:{}_true_theta'.format(key, i)] = run[2]
...
def run_experiment(args, rng):
    ...
    vnce_runs = {str(i): [] for i in sample_sizes}
    nce_runs = {str(i): [] for i in sample_sizes}
    mle_runs = {str(i): [] for i in sample_sizes}
    max_run_time = 0
        ...
        for j, sample_size in enumerate(sample_sizes):
            ...
            for k in range(args.num_random_inits):
                ...
                optimiser = VNCEOptimiserWithAnalyticExpectations(model=model, noise=noise, noise_samples=Y,
                                                                  ...
                                                                  E5=E5, nu=args.nu, eps=10 ** -20, rng=rng)
            ...
            vnce_runs[str(sample_size)].append(vnce_rnd_init_runs[np.argmin(get_final_mses(vnce_rnd_init_runs))])
            nce_runs[str(sample_size)].append(nce_rnd_init_runs[np.argmin(get_final_mses(nce_rnd_init_runs))])
            mle_runs[str(sample_size)].append(mle_rnd_init_runs[np.argmin(get_final_mses(mle_rnd_init_runs))])

    return sample_sizes, vnce_runs, nce_runs, mle_runs, max_run_time


def plot1(ax, quant_to_run_dict, sample_sizes, colour, label, marker, markersize):
    for quantile, runs in quant_to_run_dict.items():
        ...
        if quantile == '0.5':
            ...
            ax.plot(np.log10(sample_sizes), np.log10(final_mses_no_scaling_param), c=colour, alpha=0.5, marker=marker, linestyle='--', markersize=markersize)
...
def plot2(ax, quant_to_run_dict, sample_sizes, colour, label, marker, markersize):
    for quantile, runs in quant_to_run_dict.items():
        ...
        if quantile == '0.5':
            ...
            ax.plot(np.log10(sample_sizes), np.log10(final_mses_scaling_param), c=colour, alpha=0.5, marker=marker, markersize=markersize, linestyle='--')
    # ax.set_xlabel('log 10 sample size')
...
def plot_asymptotic_efficiency_curves(save_dir, sample_sizes, vnce_runs, nce_runs, mle_runs, max_run_time):
    ...
    vnce_quantile_to_run_dict = make_quantile_to_run_dict(vnce_runs, sample_sizes)
    nce_quantile_to_run_dict = make_quantile_to_run_dict(nce_runs, sample_sizes)
    mle_quantile_to_run_dict = make_quantile_to_run_dict(mle_runs, sample_sizes)
    ...
    fig1.savefig(os.path.join(save_dir, 'sample-size-against-mse.pdf'))
    ...
    fig2.savefig(os.path.join(save_dir, 'sample-size-against-mse-scaling-param.pdf'))
...
# def plot_tradeoff_curves(save_dir, sample_sizes, vnce_runs, nce_runs, mle_runs, max_run_time):
#
...
#     vnce_median_min_mses, vnce_upper_bounds = make_tradeoff_curves(tradeoff_times, vnce_runs, sample_sizes=sample_sizes)
#     nce_median_min_mses, nce_upper_bounds = make_tradeoff_curves(tradeoff_times, nce_runs, sample_sizes=sample_sizes)
#     mle_median_min_mses, mle_upper_bounds = make_tradeoff_curves(tradeoff_times, mle_runs, sample_sizes=sample_sizes)
#
...
def main(args, START_TIME):
    ...
    config = vars(args)
    config.update({'sample_sizes': results[0], 'start_time': START_TIME, 'end_time': END_TIME, 'random_seed': DEFAULT_SEED})
