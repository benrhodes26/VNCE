...
class StarsAndMoonsDataProvider(DataProvider):

    def __init__(self, c, which_set='train', size=1000, batch_size=100, max_num_batches=-1, shuffle_order=True, truncate_gaussian=False, rng=None):
        model = StarsAndMoonsModel(c=c, truncate_gaussian=truncate_gaussian, rng=rng)
        _, inputs = model.sample(n=size)
        inputs = inputs.astype(np.float32)
        self.mean = inputs.mean()
        self.std = inputs.std()
        inputs = (inputs - self.mean) / self.std
        targets = np.zeros((size, 4))  # don't need targets

        # pass the loaded data to the parent class __init__
        super(StarsAndMoonsDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(StarsAndMoonsDataProvider, self).next()
        return inputs_batch, targets_batch
