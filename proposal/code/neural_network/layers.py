...
class LeakyReluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        where a ~ 0.1, for instance.
        """
        a = 0.1
        return (inputs > 0) * inputs + (inputs <= 0) * a * inputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        a = 0.1
        return (inputs > 0) * grads_wrt_outputs + (inputs <= 0) * a * grads_wrt_outputs
...
class ELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `y = max(0, x)`.
        """
        return (inputs > 0) * inputs + (inputs <= 0) * (np.exp(inputs) - 1)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        gradients with respect to the layer inputs.
        """
        return (inputs > 0) * grads_wrt_outputs + (inputs <= 0) * np.exp(inputs) * grads_wrt_outputs
...
class SELULayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        return (inputs > 0) * l * inputs + (inputs <= 0) * a * l * (np.exp(inputs) - 1)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        a = 1.6732632423543772848170429916717
        l = 1.0507009873554804934193349852946
        return (inputs > 0) * l * grads_wrt_outputs + (inputs <= 0) * a * l * np.exp(inputs) * grads_wrt_outputs
