...
class MixtureOfTwoGaussians(LatentVarModel):
    ...
    phi(u; theta) = (1/2)*Z*N(u; 0, sigma0) + (1/2)*(1-Z)N(u; 0, sigma1)
    where Z is a binary latent variable
    """
...
class SumOfTwoUnnormalisedGaussians(Model):
    """Sum of two unnormalisedGaussians (no latent variable) given by :
    phi(u; theta) = e^c*(N(u; 0, sigma0) + N(u; 0, sigma1))"""
    ...
    def __call__(self, U):
        ...
        U = U.reshape(-1)
        a = np.exp(-self.theta[0])  # scaling parameter
        term_1 = np.exp(-U**2 / (2 * self.theta[1]**2))
    ...
    def grad_log_wrt_params(self, U):
        ...
        a = np.exp(-U**2 / (2 * self.theta[1] ** 2))
        b = np.exp(-U**2 / (2 * self.sigma1 ** 2))
        c = a / (a + b)  # (n, 1)

        d = (U**2 / self.theta[1]**3)  # (n, 1)
